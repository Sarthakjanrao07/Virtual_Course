...
function ViewCourse() {
  ...
  const handleEnroll = async (courseId, userId) => {
    ...
    if (selectedCourseData?.price === 0 || selectedCourseData?.price === "0" || String(selectedCourseData?.price).toLowerCase() === "free") {
      const CustomToast = ({ closeToast }) => (
        <div>
          <p className="mb-3 text-sm">This course is free. Do you want to enroll directly?</p>
          <div className="flex gap-2">
            <button
              ...
              onClick={async () => {
                closeToast();
                try {
                  const res = await axios.post(serverUrl + "/api/payment/enroll-free", { courseId, userId }, { withCredentials: true });
                  setIsEnrolled(true);
                  toast.success(res.data.message);
                } catch (err) {
                  toast.error(err.response?.data?.message || "Failed to enroll free course");
                }
              }}
            >
              Confirm
